...
function Path(props) {
    ...
    const returnError = props.returnError;

    if (props.returnError === false) {
        pathfinding(props.start, props.end, finalPath, processedPath, props.filters, props.returnError, props.setReturnError);
    }

    if (processedPath === 'error') {
        setReturnError(true);
    }
    if (props.returnError === false) {
